...
int main(int argc, char * argv[]) {
    ...
    // Gas setup
    TString gasmixt[6] = { "Ar", "CO2", "", "70", "30", "0" };
    TString gasname;
    parseGasName(gasmixt, gasname);
    TString GEMid = (TString)argv[1];
    ...
    // Setup primary electrons
    int total = 1;
    double x0[] = { 0};
    double y0[] = { 0};
    double z0[] = { 0.8*g.driftT };
    double e0 = 0.;
    double t0 = 0.;
    bool debug = true;
    ...
    // Prepare output file
    TString savefile = savedir + "results.root";
    ...
    for(int i=0; i<total; i++) {
        ...
        if(debug) {
            std::cout << "Progress: " << 100.*(i+1)/total << "%" << std::endl;
        }
        ...
        aval->AvalancheElectron(x0[i], y0[i], z0[i], t0, e0, vx0, vy0, vz0);
